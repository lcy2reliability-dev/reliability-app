...
- **Click any column header to sort.** On the three per-position / per-part tables (Repair Kit, Linked Parts and Parts Associated), clicking a column header now sorts by that column. First click sorts high to low, a second click flips to low to high, a third click returns to the original order. A ▼ / ▲ arrow on the header shows which column and direction is active.
- **Bin sorts down the aisles.** Because the bin format (A01-B-12) sorts naturally, sorting a Repair Kit by Bin ascending walks you A01-A-01, A01-B-01, A02, … in a single route through the stores, so refurbishing a part is one pass instead of back-and-forth. APN, class and description use the same natural alphanumeric sort; quantity sorts numerically.
- **Sort and filter stack.** Sorting keeps any per-column filters applied, and reorders every row including ones a filter is currently hiding.
- **Ephemeral by design.** Leaving and reopening the page resets to the original order; nothing about the sort is saved.
- On mobile the Repair Kit also gets a row of sort chips, because the table collapses to cards there and the header row is no longer tappable.
- Note: a sort function existed in the code but had never been wired to any header, so clicking a header did nothing. This release finishes and upgrades it (three-state, natural sort, arrow indicator).

### Council Audit Round 7 fixes
- **Repair Kit "Unlink" now respects view-only access.** unlinkComponent gained the same write-permission check as every other mutating action, so a look-only user can no longer remove a component.
- **Scan-to-fill is more reliable.** The scanner now captures its target field before it closes, and clears its state cleanly on close, so a scan always lands in the right box and a cancelled scan leaves nothing dangling.
- **Repair Kit "Qty" column relabelled "Qty/repair"** to make clear it is the quantity needed per repair, not stock on hand. (The short mobile sort chip stays "Qty".)
- **Thermal-anomaly dialog button relabelled.** The "Dismiss" button is now "Save without note": it always saved the reading, so the old label was misleading. The behaviour is unchanged.
- **Bigger touch targets in the Repair Kit.** The Edit Qty and Unlink buttons now meet the 44px minimum tap size.
- **Faster detail pages.** Opening a position, part or route now fetches its cached data and its image in parallel rather than one after another, and images are cached for the session, so repeat views are quicker.
- **Turning off "Repairable" now warns you** if the part already has a Repair Kit, so you do not hide a populated kit by accident.
- **Accessibility:** every scan button now carries a screen-reader label ("Scan QR or barcode").
- **Link-part attempt counter resets** when the popup opens, so the "ask an admin to add this APN" prompt counts attempts correctly per session.

### Image cache completed (follow-up during this release)
- Saving a route that has a picture (saveManualRoute) and auto-registering a new position from a job (saveWork) now refresh the caches immediately. Without this, an edited route image could show the old picture until something else cleared the cache, and a newly auto-registered position could take up to 60 seconds to appear. Both now update straight away.
...
### Performance + search fixes (follow-up, 2026-08-28)
- **Faster position pages — the parts tables no longer lag.** Earlier the same day the three per-position parts tables (Repair Kit, Linked Parts, Parts Associated) were changed to collapse into stacked cards on the phone. On a position with a long associated-parts list that card layout made the page slow to open and jerky to scroll. They are plain tables again that scroll sideways on the phone, exactly like the main Parts browser at the top of the Parts tab, which is fast. Desktop is unchanged.
- **Sorting still works, and Bin now sorts down the aisles.** You sort by tapping a column header (this works on the phone too now that the tables are plain). The separate mobile "Sort:" chip bars are gone along with the cards. Sorting by Bin now understands the bin format (e.g. A03-E-07): it orders by section, then aisle, then bay, then shelf, so Bin ascending walks the stores in one pass. Bins that do not match the format, or blank bins, sort to the end.
- **The phone keyboard's Go / Search key now runs the search.** Each search box (Home global search, Positions, CBM Routes) is wrapped in a form, so pressing Go / Search / Enter on the phone keyboard runs the search and closes the suggestions list. This mainly fixes the Home search, which has no Search button, where the phone keyboard's key previously did nothing.
...
**Status:** Core v1.18 deployed and verified live 2026-08-14. All the 2026-08-17 batches below — job-card display, FWO parts no longer auto-linked, user-account fixes, admin Remove-user, session security, Terms & Conditions / Privacy Policy acceptance, guided-tour and Job-list wording, the "does not replace APM" clarity, the rename to "RME Shift Personal Tracker", the full guided-tour rewrite with a role-aware Help & Reference page and FAQ, switching thermal anomaly detection from the mean to the median, and tightening the technician/admin permission split (screenshot ownership, field locking, and making decommission, delete, reactivate and creating new records admin-only) — were deployed and verified live on 2026-08-17. A further small field-feedback batch (see "Field-feedback refinements" immediately below) is built and verified in the working copy, plus the **2026-08-21** batch immediately below it (Repair Kit for repairable parts, QR/barcode scan added to the Link Part popup, the raise-a-job wizard, and the Add / Edit Position, Part and Route forms) — all **deployed and verified live on 2026-08-27** (the displayed version stayed 1.18). Firebase unchanged — still open (rules not yet published). **Superseded by v1.19 (2026-08-28), see the section above.**

### Repair Kit + expanded QR / barcode scan (2026-08-21, deployed & verified live 2026-08-27)
- **Repair Kit on repairable parts.** Any part flagged **Repairable** in the catalogue now has a **Repair Kit** section on its detail page, with a **+ Link Part** button that opens the same "search by APN" popup used for linking parts to positions — including the 📷 QR / barcode scan button, so you can scan a component straight into the kit. The kit lists each component's APN, class, bin location, quantity per repair and description, with an APN link straight to the sub-part. The point is to record, over time, exactly which bearings, shafts, seals and sub-parts a given repairable part contains, so the next person rebuilding it can see everything they need at a glance instead of tearing it down first. Nothing is pre-populated — the catalogue holds ~697 repairable parts and we'll fill in the components as we work through them. **Edit Qty** is available to any writer; **Unlink** requires either an admin or the technician who added that component (mirrors the linked-parts ownership pattern introduced in v1.16). Self-linking and duplicates are blocked. As with the standard Link Part flow, an APN not in the catalogue after two attempts prompts the technician to ask an admin to add it first. The kit table carries the same **per-column filter row** as the Linked Parts table (filter by APN, class, bin, qty or description) - it is, in essence, the same kind of parts list.
- **QR / barcode scan on four more places.** The camera scan button (📷) now appears next to the input on: (1) the **Link Part popup** (link a catalogue part to a position by scanning the APN); (2) the **raise-a-job wizard** parts search (scan the APN of a part you'll need); (3) the admin **Add / Edit Position, Part and Route** forms (position conveyor number, part APN, route start and end points); and (4) the new **Repair Kit "+ Link Part" popup** (scan a sub-part's APN to add it to a repairable part's kit — see the Repair Kit bullet above). The existing scanners on Home global search, the Positions tab and the Link Position popup are unchanged. All scans still use the browser's native BarcodeDetector when available with ZXing as a fallback, and each one flows the scanned value straight back into the input plus the field's own search or lookup handler where relevant.
- **Filter box on a part's Linked Positions.** When a part is linked to more than one position, the Linked Positions card now shows a small filter box above the chips so you can quickly narrow a long list (matches on the position name / number). Single-position parts are unchanged.
- **What's New (v1.18) banner** bumped (`WHATS_NEW_REV` → `1.18.2`) with two new bullets at the top of the list so both features get a heads-up prompt to existing users. The displayed app version stays 1.18 — this is a feature refresh within v1.18.

### Field-feedback refinements (2026-08-17, deployed & verified live 2026-08-27)
- **Route search results now carry a "+ Add to Job list" shortcut.** In the CBM tab, each route row's Actions cell shows a green **"+ Add to Job list"** link. It replaces the old "Record" shortcut, which was redundant (recording still lives on the route itself and on the job card). Clicking adds the route to your Job list in place and flips the link to an "in Job list" confirmation without navigating away. It applies to every route type, not just thermal.
- **FAQ "Who do I contact about the app?" now names the current admins.** The answer reads "The app admins — …" and fills the list of names from whoever currently holds the admin role, falling back to "Tittus (strtt) or Alex (vladesca)" if the user list can't be read. Note: once the database is locked (rules published) technicians may no longer be able to read the user list, in which case they will simply see the fallback names rather than a live list — so keep the fallback names current.
- **Confirmed (no change needed):** the position screenshot thumbnail already shows on PM and FWO job cards, not just route cards, whenever that position has a saved SCADA screenshot. The card render adds the thumbnail slot for every job and loads the position's stored screenshot; it only appears when one exists.
- **Fixed: a technician couldn't replace or remove a screenshot they had just added themselves.** Two "add a screenshot" paths — the "Screenshot missing — click to add" quick link on a position or route, and the photo step in the raise-a-job wizard — saved the image but never recorded who added it. Without that ownership stamp the app treated the screenshot as having no owner (which means admin-only), so the technician who added it was blocked from replacing or removing it. Both paths now stamp the uploader as the owner (`equipmentImageMeta` / `routeImageMeta`), and the ownership of the screenshots already in the app was reconstructed from the Activity Log so nobody is locked out of their own uploads. The quick-add link now also respects view-only access.
...
### Guided tour and Job-list wording (follow-up, 2026-08-17)
- The guided tour and the Help page now describe how the Job list actually behaves: a job you **tick complete** is cleared automatically at the **daily 4am sweep**; anything you leave unfinished **carries over and shows in red as overdue** until you complete it (then it clears at the next 4am sweep). The Help page previously said jobs go "overdue after 12 hours", which was wrong — now corrected.
- **Fixed a tour bug:** on the "Job list" step the pop-up could run off the bottom of the screen, so the text was cut off and you could not reach Next / Back to advance. The tour now keeps every pop-up on screen so it stays fully readable.

### Made clear this does not replace APM (follow-up, 2026-08-17)
- The wording throughout now makes plain that this is a **personal shift tracker, not a replacement for APM**. You **add** a PM or FWO to **your own Job list** for the shift — the old wording said "raise a job", which sounded like APM. **Nothing you do here is sent to or recorded in APM:** you keep completing your APM checklists and booking labour and parts in APM exactly as you do now.
- This message now appears everywhere you might wonder about it: a **bold notice on the "Set up your login" screen**, a line inside the **Add to Job list** wizard, the guided **tour**, the **What's New** note and the **Help** page.

### Renamed to "RME Shift Personal Tracker" (follow-up, 2026-08-17)
- The app is now called **RME Shift Personal Tracker** everywhere: the browser tab, the sign-in screen, the header (shown on desktop; it was already hidden on phones to keep the top bar clean), the Help page, the printed route-report footer, and both the **Terms & Conditions** and **Privacy Policy**. The earlier names ("RME App", "RME CBM Reliability App", "LCY2 Reliability App") no longer appear in the app or the policy documents.
- This is a name change only — nothing about how the app works has changed. The Terms and Privacy version stays 1.0 (the documents are still with HR/Legal; the version will be bumped at sign-off).


### Guided tour rewrite, Help & Reference overhaul and FAQ (follow-up, 2026-08-17)
- **The guided tour is now a complete walk-through of everything a technician can do.** It gains a short **Welcome** (with the "this does not replace APM" message up front), a dedicated **Recording CBM readings** step (open a thermal route → Record New Readings → Motor and Gearbox temperatures → Save, with a reminder to also enter them in APM), and an **Anomaly flags & Open Alerts** step explaining what gets flagged and where the alerts collect. The search step now mentions the **wildcard `*`**, the detail step explains **adding photos**, and the FWO wording is corrected (a FWO is one **assigned to you in APM** — the app note is your on-the-floor reference for the equipment, instructions and parts; and if you spot something new, add a job with a note to make raising the FWO in APM easier).
- **Help & Reference rewritten to be comprehensive and role-aware.** Technicians now see only what they can actually do; the admin-only items (Add New, Manage Users, Activity Log, Manage Inactive, Recently Deleted) appear as a separate section for admins only, and the tips are filtered the same way. New sections cover getting around the four tabs, recording readings and alerts, and your account (stay-signed-in, shared device, 7-day PIN re-lock, Change / Forgot PIN). The CBM routes section now states clearly that **thermal readings still have to be entered into APM in the route's checklist** — the app does not send anything to APM.
- **FAQ added to the bottom of the Help page** (rather than a separate menu item): 13 plain-language questions and answers covering APM, PM vs FWO, when jobs clear, recording readings, anomaly alerts, editable fields, linked vs associated parts, photos, searching, staying signed in, PINs, refreshing, and who to contact (the LCY2 RME Reliability Team — Tittus or Alex).

### Thermal anomaly detection now uses the median (follow-up, 2026-08-17)
- Thermal alerts previously compared a new reading against the **mean (average)** of a component's history. They now compare against the **median** — the component's typical reading. The median ignores the odd one-off spike (which is exactly what you are trying to catch), so a single past fault reading can no longer inflate the baseline and hide the next real fault. The 10 °C threshold and the ISO 13373 basis are unchanged.
- The change is applied consistently everywhere the statistic is used: the alert detector, the live green / amber / red tint while you type a reading, the reading hint next to each field, and the "Median" column in the printed route report. All on-screen wording that said "avg" or "baseline" now reads "median".


### Clearer split of what technicians and admins can change (follow-up, 2026-08-17)
This tightens who can change what, so the shared data stays trustworthy while every technician can still do their job and **fix their own mistakes**. Nothing here changes what the app does day to day for the people already using it well; it mainly stops accidental edits and puts the structural changes in admins' hands. No version bump (stays 1.18) and — by request — it is **not** mentioned in the What's New banner.

- **Positions and routes.** A technician can **add a screenshot** when there isn't one, **replace a screenshot they added themselves**, and **add notes** (their own notes can be edited or deleted, as before). The descriptive fields — the ID / number, type, description, and for routes the waypoints and the temperature-reading-required toggle — are now **admin-only**. If one of those is wrong, a technician asks an admin (Tittus / strtt or Alex / vladesca) to correct it.
- **Parts.** A technician can edit the **Bin Location** and the **Repairable** toggle, and add notes. The **APN** is now **admin-only** (it still keeps the old APN searchable as a "Previous APN" when an admin changes it). Everything else stays system-managed from the stores database as before.
- **Screenshot ownership.** (1) If a position, route or part has no screenshot yet, any technician can add one and it is stamped with their name. (2) Once a screenshot exists, only the technician who added it — or any admin — can replace it, so nobody's photo is overwritten by accident. (3) Older screenshots with no recorded owner can only be replaced by an admin.
- **Decommission, delete and reactivate are admin-only.** Marking a position, route or part inactive (the Active toggle), deleting it permanently, and reactivating it are all restricted to admins. (The Active toggle and Delete button no longer appear in the Edit screen for technicians.)
- **Creating new records is admin-only, with a helpful nudge.** If a technician tries to add a **FWO for a position that is not yet in the app**, or to **link a part whose APN is not in the catalogue**, the app no longer creates it silently — it shows a short message asking them to contact an admin (Tittus / strtt or Alex / vladesca) to add it first. The "Add New" create forms are admin-only too.
- **What technicians can still do, unchanged:** add PMs and FWOs to their own Job list, record thermal readings, add / edit / delete their own notes, link existing catalogue parts to positions and unlink links they made, add and replace their own screenshots, and correct a job on their own list.
- **Docs updated to match.** The guided tour, the Help & Reference page and the FAQ now describe this split. Help sections and the FAQ are role-aware: technicians see what they can do, and admins additionally see the fuller picture (editing every field, decommissioning, deleting and creating records). New FAQ entries cover "Why can't I change the ID, type or description?", "I made a mistake — can I fix it myself?", "Can I change a screenshot someone else added?" and "The position or part I need isn't in the app."

...
**Status:** Built and verified in the working copy (bracket-balanced, clean syntax build). **Awaiting deploy.** This is a feature release: the headline is sortable table columns, bundled with a full round of audit fixes (Council Audit Round 7) and the completion of the session image cache. Firebase unchanged, still open (rules not yet published).
